resource "azurerm_policy_definition" "diag-eh-neu" {
  for_each = { for diag in var.diag : diag.name => diag}

  name                = "${each.value.name}-neu"
  policy_type         = "Custom"
  mode                = "All"
...
{    "DataBricksResourceTagName":{
    ...
    "eventHubName": {
        ...
        "defaultValue": "${var.eventhub_regions.neu.eh_name}"
    },
    "eventHubRuleId": {
        "type": "String",
        "metadata": {
        ...
        "defaultValue": "${var.eventhub_regions.neu.eh_id}"
    },
    "azureRegions": {
        "type": "Array",
        "metadata": {
...
resource "azurerm_policy_definition" "diag-eh-neu-allmetrics" {
  for_each = { for diag in var.diag_all_metrics_only : diag.name => diag}

  name                = "${each.value.name}-neu-allmetrics"
  policy_type         = "Custom"
  mode                = "All"

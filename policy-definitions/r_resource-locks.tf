resource "azurerm_policy_definition" "resource-lock-def" {
  name                = "indrsrc-lock-policy-def"
  policy_type         = "Custom"
  mode                = "Indexed"
  display_name        = "${var.custom_prefix} Enforces resource locks on Production AnF resources"
  ...
  parameters = <<PARAMETERS
       {
        ...
        "defaultValue": "anf-lock_required"
      },
      "tagValue": {
        "type": "String",
        "metadata": {

resource "azurerm_policy_definition" "diag-eh-weu" {
  for_each = { for diag in var.diag : diag.name => diag}

  name                = "${each.value.name}-weu"
  policy_type         = "Custom"
  mode                = "All"
...
{    "DataBricksResourceTagName":{
    ...
    "eventHubName": {
        ...
        "defaultValue": "${var.eventhub_regions.weu.eh_name}"
    },
    "eventHubRuleId": {
        "type": "String",
        "metadata": {
        ...
        "defaultValue": "${var.eventhub_regions.weu.eh_id}"
    },
    "azureRegions": {
        "type": "Array",
        "metadata": {
...
resource "azurerm_policy_definition" "diag-eh-weu-allmetrics" {
  for_each = { for diag in var.diag_all_metrics_only: diag.name => diag}

  name                = "${each.value.name}-weu-allmetrics"
  policy_type         = "Custom"
  mode                = "All"

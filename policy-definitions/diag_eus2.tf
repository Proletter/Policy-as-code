resource "azurerm_policy_definition" "diag-eh-eus2" {
  for_each = { for diag in var.diag : diag.name => diag}

  name                = "${each.value.name}-eus2"
  policy_type         = "Custom"
  mode                = "All"
...
{   
    "isDataBricksResourceTagName":{
        ...
        "metadata": {
          ...
          "description": "The Tag name of the databricks related resources to exclude (i.e. databricks-environment etc.)"
        },
...
resource "azurerm_policy_definition" "diag-eh-eus2-allmetrics" {
  for_each = { for diag in var.diag_all_metrics_only : diag.name => diag}

  name                = "${each.value.name}-eus2-allmetrics"
  policy_type         = "Custom"
  mode                = "All"
